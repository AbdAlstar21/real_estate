...
class RealtyFavNum extends StatefulWidget {
  @override
  _RealtyFavNumState createState() => _RealtyFavNumState();
}

class _RealtyFavNumState extends State<RealtyFavNum> {
...
// List<RealtyData>? realtyListall;
  ...
  void getDataRealty(int count, String strSearch) async {
    loadingList = true;
    setState(() {});
    List arr = await getData(count, "realtys/readrealty_user_fav_num.php", strSearch, "user_id=${G_user_id_val}&");
    for (int i = 0; i < arr.length; i++) {
...
rep_id: arr[i]["rep_id"],
  ...
  Widget _appBarTitle = const Text("إدارة القارات");
