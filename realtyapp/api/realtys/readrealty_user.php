...
if (
   ...
   is_numeric($_GET["cat_id"]))
	...
	if(trim($txtsearch) != "")
	{
		...
		from realtys where realty_short_title like ? $sqlWhere  order by realty_id desc limit $start , $end";
	...
	else
	{
		...
		from realtys where  1 = 1 $sqlWhere order by realty_id desc limit $start , $end";
