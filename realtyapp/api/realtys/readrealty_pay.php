...
if (
   ...
   is_numeric($_GET["cat_id"]))
	...
	if(trim($txtsearch) != "")
	{
		...
		from realtys where realty_type='بيع' and realty_short_title like ? $sqlWhere  order by realty_id desc limit $start , $end";
	...
	else
	{
		...
		from realtys where realty_type='بيع' and  1 = 1 $sqlWhere order by realty_id asc limit $start , $end";
