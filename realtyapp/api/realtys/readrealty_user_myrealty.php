...
if (
   ...
   is_numeric($_GET["cat_id"]))
	...
	if(trim($txtsearch) != "")
	{
		...
		from realtys where user_id = $user_id and realty_short_title like ? $sqlWhere  order by realty_id desc limit $start , $end";
	...
	else
	{
		$sql = "select * 
		, (select fav_id from favorite where realty_id = realtys.realty_id and user_id = $user_id ) as fav_id 
		from realtys where  user_id = $user_id $sqlWhere order by realty_id desc limit $start , $end";
